...
const erc20ContractJson = require('../../abis/erc20.json')
const contractJson = require('../../abis/zkbToken.json')
 ...
 */
export async function deposit({ currency, amount, chainId }) {
  const deposit = createDeposit({ nullifier: rbigint(31), secret: rbigint(31) })
  const note = toHex(deposit.preimage, 62)
  const noteString = `zkborrow-${currency}-${amount}-${chainId}-${note}`
...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  let erc20tornadoJson, tornadoAddress, tokenAddress
