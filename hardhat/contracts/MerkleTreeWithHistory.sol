...
contract MerkleTreeWithHistory {
    ...
    mapping(address => MerkleTree) public merkleTreeByJToken;
    ...
    function initializeTree(address _jToken) external {
        require(msg.sender == admin, "msg.sender should be admin");
        require(levels > 0, "_levels should be greater than zero");

        for (uint32 i = 0; i < levels; i++) {
            merkleTreeByJToken[_jToken].filledSubtrees[i] = zeros(i);
        }

        merkleTreeByJToken[_jToken].roots[0] = zeros(levels - 1);
    }
  ...
  */
    ...
    function _insert(bytes32 _leaf, address _jToken)
        external
        returns (uint32 index)
    {
        require(
            joetroller.isMarketListed(msg.sender),
            "can only be called by markets"
        );
        uint32 _nextIndex = merkleTreeByJToken[_jToken].nextIndex;
        require(
        ...
        for (uint32 i = 0; i < levels; i++) {
            if (currentIndex % 2 == 0) {
                ...
                right = zeros(i);
                merkleTreeByJToken[_jToken].filledSubtrees[
                    i
                ] = currentLevelHash;
            } else {
                left = merkleTreeByJToken[_jToken].filledSubtrees[i];
        ...
        uint32 newRootIndex = (merkleTreeByJToken[_jToken].currentRootIndex +
            1) % ROOT_HISTORY_SIZE;
        merkleTreeByJToken[_jToken].currentRootIndex = newRootIndex;
        merkleTreeByJToken[_jToken].roots[newRootIndex] = currentLevelHash;
        merkleTreeByJToken[_jToken].nextIndex = _nextIndex + 1;
        return _nextIndex;
    }
  ...
  */
    function isKnownRoot(bytes32 _root, address _jToken)
        public
        view
        returns (bool)
    {
        ...
        uint32 _currentRootIndex = merkleTreeByJToken[_jToken].currentRootIndex;
        uint32 i = _currentRootIndex;
        mapping(uint256 => bytes32) storage _roots = merkleTreeByJToken[_jToken]
            .roots;
        do {
            if (_root == _roots[i]) {
                return true;
  ...
  */
    function getLastRoot(address _jToken) public view returns (bytes32) {
        MerkleTree storage tree = merkleTreeByJToken[_jToken];
